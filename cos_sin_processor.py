...
class CosSinProcessor(PhysicalSystemWrapper):
    ...
    def set_physical_system(self, physical_system):
        ...
        low = np.concatenate((np.delete(physical_system.state_space.low, self._remove_idx), [-1.0, -1.0]))
        high = np.concatenate((np.delete(physical_system.state_space.high, self._remove_idx), [1.0, 1.0]))
        self.state_space = gymnasium.spaces.Box(low, high, dtype=np.float64)

        self._limits = np.concatenate((np.delete(physical_system.limits, self._remove_idx), [1.0, 1.0]))
        self._nominal_state = np.concatenate((np.delete(physical_system.nominal_state, self._remove_idx), [1.0, 1.0]))
